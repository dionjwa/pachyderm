...
func (d *driver) deleteCommit(ctx context.Context, commit *pfs.Commit) error {
	...
	// inspect 'commit' to retrieve the commit ID (we'll read it again in a txn)
	commitInfo, err := d.inspectCommit(ctx, commit, false)
	if err != nil {
		return err
	}
	...
	if _, err := col.NewSTM(ctx, d.etcdClient, func(stm col.STM) error {
		...
		deleteCommit := func(repo col.ReadWriteCollection, commit *pfs.Commit) error {
			// Store commitInfo in 'deleted' and remove commit from etcd
			commitInfo := &pfs.CommitInfo{}
			if err := repo.Get(commit.ID, commitInfo); err != nil {
				return err
			}
			...
			// commit being removed, in which case we're subtracting too much. Doing
			// the right thing is hard, though. Hopefully nobody ends up with a
			// negative repo size and cares too much
			repoInfo := &pfs.RepoInfo{}
			if err := repos.Get(commit.Repo.Name, repoInfo); err != nil {
				return err
			}
	...
	// Rewrite all commits in affected repos so that:
	...
	visited := make(map[string]struct{}) // repos we've already visited
	for _, deletedCommit := range deleted {
		repo := deletedCommit.Commit.Repo.Name
		if _, ok := visited[repo]; ok {
			continue
		}
		visited[repo] = struct{}{}
